...
Even with **Segregation of Concerns** (UI-layer vs. Business-logic-layer), often injection is needed in React components. For that, `useInject` and `useInjectDeferred` hooks are provided by `@lensapp/injectable-react` package.
